...
export default function SubscriptionPage() {
  ...
  async function requestRedemption() {
    ...
    try {
      ...
      if (redeemType === "free_station" || redeemType === "station_parts") {
      ...
      if (res.ok) {
        ...
        try {
          const rdRes = await apiFetch("/api/redemptions");
          const rdData = await rdRes.json();
          if (rdData.redemptions) setRedemptions(rdData.redemptions);
        } catch (refreshErr) {
          console.error("Error refreshing redemptions:", refreshErr);
        }
      } else {
  ...
  return (
    <DashboardShell title="Subscription & Payments">
      <div className="space-y-8">
        {/* Account Status Header */}
        <div className="glass-card rounded-2xl p-6 bg-gradient-to-r from-amber-400/10 to-orange-500/5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-white">Subscription & Payments</h2>
              <p className="text-slate-400 mt-1">Manage your monthly fees and redeem earned points.</p>
            </div>
        ...
        <section className="space-y-4">
          <h3 className="text-lg font-bold text-white">Monthly Station Fee</h3>
