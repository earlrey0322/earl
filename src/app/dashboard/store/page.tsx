    ...
    <DashboardShell title="PSPCS Store">
      <div className="space-y-8">
        <div className="glass-card rounded-2xl p-6 bg-gradient-to-r from-amber-400/10 to-orange-500/5">
          ...
          <p className="text-green-300 mt-1">Buy charging stations and parts</p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div><span className="text-green-300">Number:</span> <span className="text-white font-bold">09469086926</span></div>
            <div><span className="text-green-300">Name:</span> <span className="text-white font-bold">Earl Christian Rey</span></div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {items.length === 0 ? (
            <p className="text-green-300">Loading items...</p>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {items.map((item) => (
                <div key={item.id} className="glass-card rounded-2xl overflow-hidden">
                  <div className="relative bg-gradient-to-br from-green-950 to-green-950 p-8 flex items-center justify-center">
                    <img
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-sm text-green-300 mt-1">{item.description}</p>
          ...
          <div className="space-y-4">
            ...
            <div className="glass-card rounded-2xl p-6">
              <div className="space-y-3 max-h-[300px] overflow-y-auto">
                {orders.map((order) => (
                  <div key={order.id} className="p-4 bg-green-950/50 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-green-300 mt-1">Ref: {order.reference_number}</p>
                        <p className="text-xs text-green-400">{new Date(order.created_at).toLocaleDateString()}</p>
                      </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={form.fullName} onChange={(e) => setForm(p => ({ ...p, fullName: e.target.value }))}
                    className="w-full px-4 py-3 bg-[#0a1f0f] border border-green-700 rounded-lg text-white" placeholder="Your full name" />
                </div>
                <div>
                  ...
                  <input type="text" value={form.contactNumber} onChange={(e) => setForm(p => ({ ...p, contactNumber: e.target.value }))}
                    className="w-full px-4 py-3 bg-[#0a1f0f] border border-green-700 rounded-lg text-white" placeholder="09XX XXX XXXX" />
                </div>
                <div>
                  ...
                  <textarea value={form.deliveryAddress} onChange={(e) => setForm(p => ({ ...p, deliveryAddress: e.target.value }))}
                    className="w-full px-4 py-3 bg-[#0a1f0f] border border-green-700 rounded-lg text-white" rows={3} placeholder="Full delivery address" />
                </div>
                <div>
                  <label className="block text-sm text-green-200 mb-1">GCash Reference Number</label>
                  <input type="text" value={form.referenceNumber} onChange={(e) => setForm(p => ({ ...p, referenceNumber: e.target.value }))}
                    className="w-full px-4 py-3 bg-[#0a1f0f] border border-green-700 rounded-lg text-white" placeholder="Reference number" />
                </div>
              </div>
